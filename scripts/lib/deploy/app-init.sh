...
deploy_app_wait_for_mysql() {
    ...
    until php -r '
        ...
        $user = getenv("DB_USERNAME") ?: "xflickr";
...
deploy_app_first_boot() {
    ...
    deploy_artisan_run db:seed --class=Modules\\Auth\\Database\\Seeders\\AdminUserSeeder --force --no-interaction || return 1
    deploy_artisan_run config-store:ensure-index --no-interaction || return 1
    deploy_artisan_run events:install-indexes --no-interaction || return 1
}
