...
final class FlickrAppProfileControllerTest extends TestCase
{
    ...
    public function test_saved_flickr_app_appears_on_connections_hub(): void
    {
        ...
        $response = $this->get('/connections?provider=flickr');
        $response->assertOk();
        $response->assertInertia(fn ($page) => $page
            ->component('Connections/Index')
            ->has('flickr_apps', 1)
            ->where('flickr_apps.0.profile', 'main')
            ->where('flickr_apps.0.label', 'Saved app')
            ->where('flickr_apps.0.api_key_hint', 'test…2345'));
    }
