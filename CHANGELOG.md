...
- Audit 260714 (N1–N10): Services never call Eloquent directly (`ServiceLayeringTest`); reusable model scopes (`forConnection` / status / `forAccount` / `forOwner`) in repositories; contacts URL import is `POST /api/v1/flickr/accounts/{connection}/contacts` with `{source:"url"}` (removed imperative `/import`); `ApiV1RouteShapeTest` verb denylist expanded; `saveAppProfile` takes typed DTOs; OAuth success/failure logs live in `FlickrOAuthService` (fingerprints only); `ConnectionPublicIdService` under `Services/`; frontend `Components/` has zero flat files (`Layout/` PascalCase); queue FQCN-rename runbook in known-dangerous-commands; conscious skip of `laravel-activities` recorded.
- Audit 260714_2 residuals: `ServiceLayeringTest` fails closed on `Model::where` / `::find` / `::with` / etc. (static-receiver allowlist); contacts URL import returns `ContactImportResult` DTO.
- Audit 260714_3 complexity reduction: removed dead `ContactsService` facade (peer Contacts `Services\` imports forbidden); merged contact catalog/download/detail stats into `ContactStatsService` and folded `ContactDetailService` into the contact show controller; merged transfer read queries into `TransferQueryService`; merged Operations dashboard/ops snapshots into `SnapshotService`; consolidated eight crawl `Fetch*Job` classes into `FetchCrawlPageJob` + `TaskType` config (drain `xflickr` before deploy); moved Flickr API audit probes into `FlickrTokenHealthService::auditEndpoints`; decomposed long methods in upload/download/browse/graph/verify paths into private helpers.
...
- A6 module facades: peer modules import only `FlickrAccountsService` and `StorageService` from Flickr/Storage `Services\` (architecture test `ModuleFacadeImportTest`). Contacts has no peer facade — peer imports of `Modules\Contacts\Services\*` are forbidden. Focused services remain for internal module use.
...
- PHPStan baseline cleared to **0**: covariant `aggregateId(): string` on Flickr/Storage account events (compatible with EventSourcing `?string`); empty baseline allowed via `--allow-empty-baseline`.
