...
final class StorageBrowseSyncService
{
    ...
    public function sync(
        ...
        return DB::transaction(function () use ($account, $driver, $containerId, $parentRemoteId, $maxBatches): array {
            ...
            for ($batch = 0; $batch < $maxBatches; $batch++) {
                if ($state->albums_complete && $state->items_complete) {
                    break;
                }
                ...
                if ($state->reconciling) {
                    $this->syncStates->appendReconcileSeenRemoteIds($account->id, $parentRemoteId, $batchRemoteIds);
                }
                ...
                if ($state->reconciling && ! $wasItemsComplete && $state->items_complete) {
                    $this->finalizeReconciliation($account, $containerId);
                }
    ...
    private function finalizeReconciliation(StorageAccount $account, ?string $containerId): void
    {
        $parentRemoteId = $this->parentKey($containerId);
