    ...
    public function browse(
        ...
        if ($includeAlbums) {
            if ($albumPageToken !== null && $albumPageToken !== '') {
                $folderResponse = Http::withToken($accessToken)->get($albumPageToken);
                ...
                $folderResponse = Http::withToken($accessToken)->get(self::GRAPH_BASE.$path, array_merge($query, [
        ...
        if ($includeItems) {
            if ($itemPageToken !== null && $itemPageToken !== '') {
                $fileResponse = Http::withToken($accessToken)->get($itemPageToken);
            } else {
                $fileResponse = Http::withToken($accessToken)->get(self::GRAPH_BASE.$path, array_merge($query, [
                    '$filter' => 'file ne null',
                ]));
            }
