...
interface TransferBatchPanelProps {
    batch: TransferBatch;
    account: FlickrAccount | undefined;
}

export default function TransferBatchPanel({ batch, account }: TransferBatchPanelProps) {
    ...
    const loadFailures = useCallback(async () => {
        ...
        try {
            const response = await apiGet<{ data: BatchDetailResponse }>(
                flickrApiAccountPath(account.public_id, `/transfers/${batch.id}`),
            );
            const failed = response.data.items.filter((item) => item.status === 'failed');
            setItems(failed);
        } finally {
            setLoading(false);
        }
    ...
    const retryItem = async (flickrPhotoId: string) => {
        ...
        try {
            await apiPost(
                flickrApiAccountPath(account.public_id, `/transfers/${batch.id}/items/${flickrPhotoId}/retries`),
            );
