...
export default function DataTable<T>({
    ...
    const selectedRows = selection
        ? data.filter((row) => selection.selectedKeys.has(rowKey(row)))
        : [];
    const selectedKeys = selection ? [...selection.selectedKeys] : [];

    const bulkBar =
        selection && bulkActions && onBulkClear ? (
            <BulkActionBar
                                    ...
                                    {hasSelection ? (
                                        <td className="w-10 px-3 py-3 align-top">
                                            <Checkbox
                                                ...
                                                title={!selectable ? 'Unavailable while an operation is in progress' : undefined}
                                            />
                                        </td>
                                    ) : null}
                                    {columns.map((column) => {
